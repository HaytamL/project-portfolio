      ...
      {isSmallGrid ? (
        <div className="grid gap-12 sm:grid-cols-2 lg:grid-cols-3 place-items-center">
          {slides.map((slide, i) => (
            <div
              key={i}
              className="bg-white/10 backdrop-blur-xl border border-white/30 shadow-2xl
                         rounded-2xl p-6 w-[340px] min-h-[400px]
        ...
        <div className="flex flex-col gap-10 items-center">
          {slides.map((slide, i) => (
            <div
              key={i}
              className="bg-white/10 backdrop-blur-xl border border-white/30 shadow-2xl
                         rounded-2xl p-10 w-full max-w-5xl
                         flex flex-col md:flex-row items-center gap-10 text-white
                         transition-transform duration-300 hover:scale-[1.03] hover:bg-white/20"
            >
              {slide.img && (
